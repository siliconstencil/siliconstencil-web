export default function DemoPage() {
  const styles = {
    body: {
      ...
      position: 'relative', overflowX: 'hidden', margin: 0,
      padding: 'env(safe-area-inset-top) 20px env(safe-area-inset-bottom) 20px'
    },
    header: { borderBottom: '4px solid #1f1f23', width: '100%', maxWidth: '800px' },
    logo: { fontWeight: '800', color: '#fff', textTransform: 'uppercase', letterSpacing: '-0.05em', lineHeight: '0.85' },
    box: { backgroundColor: '#000', border: '4px solid #1f1f23', position: 'relative', width: '100%', maxWidth: '800px' },
    ...
    <div style={styles.body}>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        * { font-family: 'JetBrains Mono', monospace !important; box-sizing: border-box; }
        ...
        .logo-text { font-size: 72px; margin-bottom: 24px; }
        .brutalist-container { padding: 48px; box-shadow: 12px 12px 0px 0px rgba(255, 69, 0, 0.15); margin-top: 40px; }
        .header-wrap { padding-bottom: 24px; }
        .status-title { font-size: 28px; }
        @media (max-width: 768px) {
          .logo-text { font-size: 42px; margin-bottom: 12px; }
          .brutalist-container { padding: 24px; box-shadow: 6px 6px 0px 0px rgba(255, 69, 0, 0.15); margin-top: 24px; }
          .header-wrap { padding-bottom: 16px; }
          .status-title { font-size: 20px; }
          .console-text { font-size: 14px; }
          .warning-text { font-size: 16px; margin-top: 24px; }
          .footer-wrap { flex-direction: column; gap: 10px; align-items: flex-start !important; margin-top: 24px !important; }
        }
      `}} />
      <div className="scanlines"></div>
      <div className="grid-bg"></div>
      <main style={{ width: '100%', maxWidth: '800px', zIndex: 10, display: 'flex', flexDirection: 'column' }}>
        <header className="header-wrap" style={styles.header}>
          ...
          <p style={{ fontSize: '12px', letterSpacing: '0.4em', color: '#52525b', fontWeight: '700', textTransform: 'uppercase' }}>
            MONOLITH INFRASTRUCTURE // V4.0
          </p>
        </header>
        <section className="brutalist-container" style={styles.box}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '24px' }}>
            ...
            <h2 className="status-title" style={{ color: '#fff', fontWeight: '700', letterSpacing: '0.2em', textTransform: 'uppercase' }}>SYSTEM OFFLINE</h2>
          </div>
          <div className="console-text" style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontWeight: '700', textTransform: 'uppercase' }}>
        ...
        <footer className="footer-wrap" style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', color: '#3f3f46', fontWeight: '700', letterSpacing: '0.1em', textTransform: 'uppercase', marginTop: '40px' }}>
